...
def update(frame_idx):
    ...
    frame = first_frames.iloc[frame_idx][2:].values  # Skip frame number and time columns
    color = colors[frame_idx]  # Get the color for the current frame
    plot_skeleton(frame, ax, color)
    return ax,

# Create the animation
ani = FuncAnimation(fig, update, frames=len(first_frames), interval=1000 / frames_per_second, repeat=False)

# Save the animation as a GIF
ani.save('../output/sts_gif.gif', writer='pillow', fps=frames_per_second)
